...
using OffStreamChanges = async::Channel<PlaybackEventsMap, PlaybackParamList, bool /*flushOffstream*/>;
...
struct ArrangementContext
{
    ...
    bool hasEnd() const
    {
        return actualDuration != mpe::INFINITE_DURATON;
    }
...
private:
    ...
    void calculateActualDuration(const ArticulationMap& articulationsApplied)
    {
        ...
        if (articulationsApplied.empty() || m_arrangementCtx.nominalDuration == INFINITE_DURATON) {
            return;
        }
