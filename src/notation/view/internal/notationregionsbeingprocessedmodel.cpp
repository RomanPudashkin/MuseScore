...
static const Segment* findSegmentFrom(const mu::engraving::Score* score, const System* system,
...
void NotationRegionsBeingProcessedModel::onProgressFinished(const InstrumentTrackId& instrumentTrackId)
{
    ...
    if (wasEmpty != m_regions.empty()) {
        m_shouldShowRegions = !configuration()->optionB() || playbackController()->isPlaying();
        emit isEmptyChanged();
    }
}
...
{
    const mu::engraving::staff_idx_t staffIdx = system->firstVisibleSysStaffOfPart(part);
    const mu::engraving::SysStaff* sysStaff = system->staff(staffIdx);
    if (!sysStaff) {
        return {};
    }

    const mu::engraving::Score* score = system->score();
