...
void GridCanvas::paint(QPainter* painter)
{
    ...
    painter->setRenderHint(QPainter::Antialiasing, inspectorConfiguration()->antialiasedDrawing());

    painter->fillRect(childrenRect(), QApplication::palette().color(QPalette::Window).lighter());
    QPen pen = painter->pen();
    pen.setWidth(1);

    bool isThemeDark = inspectorConfiguration()->themeType() == IInspectorConfiguration::ThemeType::Dark;
