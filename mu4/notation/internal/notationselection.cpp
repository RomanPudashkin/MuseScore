...
INotationSelection::RangeBoundingArea NotationSelection::rangeBoundingArea() const
{
    if (!isRange()) {
        return {};
    }
    ...
    for (const RangeSection& rangeSection: rangeSections) {
        ...
        result.sections.push_back(rect);

        std::vector<QRectF> sectionStaves = this->sectionStaves(rangeSection);
        for (const QRectF& staffRect: sectionStaves) {
            result.staves.push_back(staffRect);
        }
    }
...
std::vector<QRectF> NotationSelection::sectionStaves(const NotationSelection::RangeSection& selection) const
{
    std::vector<QRectF> staves;

    const Ms::System* segmentSystem = selection.system;
    const Ms::Segment* startSegment = selection.startSegment;
    const Ms::Segment* endSegment = selection.endSegment;

    for (int i = score()->selection().staffStart(); i < score()->selection().staffEnd(); i++) {
        Ms::SysStaff* segmentStaff = segmentSystem->staff(i);

        double x1 = startSegment->pagePos().x();
        if (startSegment->measure()->first() == startSegment) {
            x1 = startSegment->measure()->pagePos().x();
        }

        double width = endSegment->pageBoundingRect().topRight().x() - x1;
        double y1 = segmentStaff->y() + startSegment->pagePos().y();
        double height = segmentStaff->bbox().height();

        QRectF rect = QRectF(x1, y1, width, height).translated(segmentSystem->page()->pos());
        staves.push_back(rect);
    }
